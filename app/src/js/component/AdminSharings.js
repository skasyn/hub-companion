...
import { submitSharingAction } from "../actions/index";
...
function MapDispatchToProps(dispatch) {
    return {
        changeStatus: sharing => dispatch(submitSharingAction(sharing)),
    }
}

class CompAdminSharing extends React.Component {
    ...
    selectedTag(status, index) {
        this.props.sharings[index].status = status;
        this.setState({changeStatus: -1});
        this.props.changeStatus(this.props.sharings[index]);
    }
    ...
    render() {
        ...
        this.props.sharings.forEach((element, key) => {
            element.key = key;
        });
        this.props.sharings.sort((a, b) => a.status - b.status);
        if (this.props.sharings.length > 0) {
            list = (
                <div className="list-activities">
                    <Table
                        ...
                        pagination={{position: this.props.sharings.length > 10 ? "bottom" : "none"}}
                    />
                </div>
            )
        }
...
const AdminSharing = connect(mapStateToProps, MapDispatchToProps)(CompAdminSharing);
